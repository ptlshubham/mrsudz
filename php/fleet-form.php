...
try {
    // Use filter_input matching your form's field names (company, name, email, phone, fleet_size, message)
$form_company = trim((string) filter_input(INPUT_POST, 'company', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
...
// fleet_size: keep only digits
$raw_fleet_size = trim((string) filter_input(INPUT_POST, 'fleet_size', FILTER_DEFAULT));
$fleet_size = preg_replace('/[^0-9]+/', '', $raw_fleet_size);
...
$recaptcha_response = isset($_POST['g-recaptcha-response']) ? trim((string) $_POST['g-recaptcha-response']) : '';
    ...
    if (!$form_name || !$contact_message || !$form_company) {
        throw new \Exception('Company name, contact name and message are required');
    }
    ...
    $sql = "INSERT INTO `fleet` (`company`, `name`, `email`, `phone`, `fleet_size`, `message`) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    if (!$stmt) {
        throw new \Exception("Prepare failed: " . mysqli_error($conn));
    }

    // 6 params: string, string, string, string, int, string => "ssssis"
    if (!mysqli_stmt_bind_param($stmt, "ssssis", $form_company, $form_name, $contact_email, $contact_phone, $fleet_size, $contact_message)) {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Thank You for Your Fleet Quote Request - Mr Sudz Car Wash';
    $mail_body = '<html><body style="font-family: Arial, sans-serif;">
        <h2>Thank You for Your Fleet Quote Request!</h2>
        <p>Dear ' . htmlspecialchars($form_name, ENT_QUOTES, 'UTF-8') . ',</p>
        <p>We’ve received your fleet quote request and our fleet specialist will contact you within 24 hours.</p>
        <ul>
            <li><strong>Company:</strong> ' . htmlspecialchars($form_company, ENT_QUOTES, 'UTF-8') . '</li>
            <li><strong>Contact Name:</strong> ' . htmlspecialchars($form_name, ENT_QUOTES, 'UTF-8') . '</li>
            ...
            <li><strong>Fleet Size:</strong> ' . htmlspecialchars($fleet_size, ENT_QUOTES, 'UTF-8') . '</li>
    ...
    echo json_encode(['success' => true, 'message' => 'Fleet quote request sent successfully']);
